...
Usage: python examples/eval/logicvista/eval_logicvista.py --model_path Qwen/Qwen2.5-VL-3B-Instruct --output_dir results/logicvista
"""
...
def load_problems(dataset_name, split):
    ...
    for idx, item in enumerate(dataset):
        ...
        problems.append({"index": idx, "prompt": [{"role": "user", "content": content}], "images": [image] if image else [],
                         "answer": answer, "question": question, "skill": skill, "category": category})
...
def main():
    ...
    for i, p in enumerate(tqdm(problems, desc="LogicVista")):
        ...
        all_results[key] = {"index": p["index"], "question": p["question"], "answer": p["answer"],
                            "response": response, "score": score, "skill": p["skill"], "category": p["category"]}
