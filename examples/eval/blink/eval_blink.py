"""
BLINK Evaluation: Multi-image visual reasoning.
HF: BLINK-Benchmark/BLINK
Usage: python examples/eval/blink/eval_blink.py --model_path Qwen/Qwen2.5-VL-3B-Instruct --output_dir results/blink
"""
...
def load_problems(dataset_name, split):
    ...
    for idx, item in enumerate(dataset):
        ...
        task_type = item.get("task_type", item.get("task_name", ""))
        img_tags = "".join(["<image>\n"] * len(images))
...
def main():
    ...
    parser.add_argument("--dataset_name", type=str, default="BLINK-Benchmark/BLINK")
    parser.add_argument("--split", type=str, default="val")
    parser.add_argument("--output_dir", type=str, default="results/blink")
    ...
    output_file = os.path.join(args.output_dir, "blink_output.json")
    scores_file = os.path.join(args.output_dir, "blink_scores.json")
    ...
    all_results = {}
    for i, p in enumerate(tqdm(problems, desc="BLINK")):
        key = str(p["index"])
        ...
        gt_letter = extract_answer_letter(p["answer"]) if len(p["answer"]) > 1 else p["answer"].strip().upper()
        score = 1.0 if pred_letter == gt_letter else 0.0
        all_results[key] = {"index": p["index"], "question": p["question"], "answer": p["answer"], "response": response, "pred_letter": pred_letter, "score": score, "task_type": p["task_type"]}
    ...
    metrics = {"blink_acc": {"accuracy": overall, "total": len(all_scores)}}
    ...
    logger.info(f"BLINK acc: {overall * 100:.2f}%")
