"""
V*Bench Evaluation: Visual search and reasoning benchmark.
HF: lmms-lab/vstar-bench
Usage: python examples/eval/vstar_bench/eval_vstar_bench.py --model_path Qwen/Qwen2.5-VL-3B-Instruct --output_dir results/vstar_bench
"""
...
def load_problems(dataset_name, split):
    ...
    for idx, item in enumerate(dataset):
        ...
        problems.append({"index": idx, "prompt": [{"role": "user", "content": content}], "images": [image] if image else [], "answer": answer, "question": question, "category": category})
...
def main():
    ...
    parser.add_argument("--dataset_name", type=str, default="lmms-lab/vstar-bench")
    parser.add_argument("--split", type=str, default="test")
    parser.add_argument("--output_dir", type=str, default="results/vstar_bench")
    ...
    output_file = os.path.join(args.output_dir, "vstar_bench_output.json")
    scores_file = os.path.join(args.output_dir, "vstar_bench_scores.json")
    ...
    all_results = {}
    for i, p in enumerate(tqdm(problems, desc="V*Bench")):
        key = str(p["index"])
        ...
        gt_letter = p["answer"].strip().upper()
        if len(gt_letter) > 1:
            gt_letter = extract_answer_letter(gt_letter)
        score = 1.0 if pred_letter == gt_letter else 0.0
        all_results[key] = {"index": p["index"], "question": p["question"], "answer": p["answer"], "response": response, "pred_letter": pred_letter, "score": score, "category": p["category"]}
    ...
    logger.info(f"V*Bench overall_acc: {overall * 100:.2f}%")
